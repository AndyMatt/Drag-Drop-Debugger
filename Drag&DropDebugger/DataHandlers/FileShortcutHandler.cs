...
namespace Drag_DropDebugger.DataHandlers
{
    ...
    public struct EnvironmentVariableLocationBlock
    {
        ...
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 260)]
        public string mPathBytes;
        public LocationBlockPathW mPathWBytes;
    }
    ...
    internal class FileShortcutHandler
    {
        Dictionary<uint, Type> BlockLookup = new Dictionary<uint, Type>()
        {
        ...
        public FileShortcutHandler(TabControl ParentTab, MemoryStream dropData, string dropType)
        {
            ...
            TabControl TabCtrl = TabHelper.AddSubTab(ParentTab,"Shortcut");

            ShellLinkHeader shellLinkHeader = BlockReader<ShellLinkHeader>.ReadBlock(shellHeaderBytes);

            Dictionary<string, Object> props = StringHelper.ClassToDictionary(shellLinkHeader);
            StackedDataTab stackedDataTab = new StackedDataTab("Properties");

            stackedDataTab.AddDataGrid("Properties", props);

            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasTargetIDList))
            {
                ReadShellItems(byteReader, TabCtrl);
            }

            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasLinkInfo))
            {
                ShellLinkInfo shellLinkInfo = ReadShellLinkInfo(byteReader);
                stackedDataTab.AddDataGrid("ShellLinkInfo", StringHelper.ClassToDictionary(shellLinkInfo));
            }

            ProcessStringDataFlags(shellLinkHeader.mDataFlags, byteReader, stackedDataTab);

            TabHelper.AddStackTab(TabCtrl, stackedDataTab);
            ...
            while (!byteReader.End() && byteReader.scan_uint() != 0)
            {
                ...
                {
                    ReadPropertyStoreDataBlock(blockbyte, TabCtrl);
                }
                else if(blockID == 0xa000000c)
                {
                    ReadShellItems(blockbyte, TabCtrl);
                }
                else if(BlockLookup.ContainsKey(blockID))
                {
                    ...
                    if (blockReader != null)
                    {
                        ...
                        if(block is not null)
                            TabHelper.AddDataGridTab(TabCtrl, block.GetType().Name, StringHelper.ClassToDictionary(block));
                    }
                }
            }
        }
        ...
        Dictionary<DataFlags, string> DataStringsFlags = new Dictionary<DataFlags, string>()
        {
        ...
        void ProcessStringDataFlags(DataFlags mDataFlags, ByteReader byteReader, StackedDataTab stackedDataTab)
        {
            Dictionary<string, object> mData = new Dictionary<string, object>();

            /*
            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasName))
            {
                ushort mDescriptionSize = byteReader.read_ushort();
                string mDescriptionW = byteReader.read_UnicodeString((int)mDescriptionSize);
                mData.Add("Description", mDescriptionW);
            }


            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasRelativePath))
            {
                ushort mRelativePathSize = byteReader.read_ushort();
                string mRelativePathW = byteReader.read_UnicodeString((int)mRelativePathSize);
                mData.Add("RelativePath", mRelativePathW);
            }

            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasWorkingDir))
            {
                ushort mWorkingDirSize = byteReader.read_ushort();
                string mWorkingDirW = byteReader.read_UnicodeString((int)mWorkingDirSize);
                mData.Add("WorkingDirectory", mWorkingDirW);
            }

            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasArguments))
            {
                ushort mArguementSize = byteReader.read_ushort();
                string mArguementW = byteReader.read_UnicodeString((int)mArguementSize);
                mData.Add("Arguements", mArguementW);
            }

            if (shellLinkHeader.mDataFlags == (shellLinkHeader.mDataFlags | DataFlags.HasIconLocation))
            {
                ushort mIconLocationSize = byteReader.read_ushort();
                string mIconLocationW = byteReader.read_UnicodeString((int)mIconLocationSize);
                mData.Add("IconLocation", mIconLocationW);
            }*/
            ...
            if(mData.Count > 0)
            {
                stackedDataTab.AddDataGrid("Data", mData);
            }
        }

        void ReadShellItems(ByteReader byteReader, TabControl parentTab)
        {
            ...
            while (!IDByteReader.End() && IDByteReader.scan_ushort() != 0)
            {
                TabControl tabControl;
                Object? obj = ShellItemHandler.Handle(parentTab, IDByteReader, out tabControl);
            }
        }
        ...
        void ReadPropertyStoreDataBlock(ByteReader byteReader, TabControl tabControl)
        {
            TabControl childTab = TabHelper.AddSubTab(tabControl, "PropertyStoreDataBlock");
            ...
            while (!propertyReader.End() && propertyReader.scan_uint() != 0)
            {
                ...
                propertySetTab.AddPropertySet(propertySet);
